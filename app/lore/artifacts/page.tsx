...
export const metadata: Metadata = {
  title: 'Artifacts - Fanhattan Lore',
  description: 'Discover the legendary artifacts of Fanhattan, mysterious items with powers that blur the line between technology and magic.',
};
...
export default async function ArtifactsPage({ searchParams }: PageProps) {
  ...
  const allArtifacts = getLoreByType('artifacts');
  const artifacts = tag ? filterLoreByTag(allArtifacts, tag) : allArtifacts;
  const tags = getTagsForType('artifacts');
      ...
      <Section className="pt-32 pb-12" container={false}>
        <div className="container mx-auto px-4">
          ...
          <p className="text-xl text-muted-foreground">
            Legendary items that shape the fate of Fanhattan
          </p>
        </div>
      </Section>
      ...
      <Section>
        ...
        {artifacts.length === 0 && (
          <div className="text-center py-12 text-muted-foreground">
            No artifacts found with tag &quot;{tag}&quot;
          </div>
        )}
      </Section>
    </>
