...
import { z } from 'zod';
import { getAllCanon, CanonEntry, CanonType } from '@/lib/content';
import { loadAllContent, createSlugIndex } from '@/lib/lore/loader';
 ...
 * Only validates world, events, myths - districts/characters/systems are validated by the lore system
 */
export function validateCanonContent(): ValidationError[] {
  const errors: ValidationError[] = [];
  const allEntries = getAllCanon();
  const entries = allEntries.filter((entry) => {
    const folder = path.basename(path.dirname(entry.filepath));
    return !LORE_VALIDATED_FOLDERS.includes(folder);
  });

  if (entries.length === 0 && allEntries.length > 0) {
    // All content is lore-validated, nothing to validate here - that's ok
    return [];
  }
  ...
  // Add lore slugs so world/events/myths can reference characters, districts, etc.
  try {
    const loreEntries = loadAllContent();
    const loreIndex = createSlugIndex(loreEntries);
    for (const slug of loreIndex.keys()) {
      validSlugs.add(slug);
    }
  } catch {
    // Lore loader may fail if content is invalid - continue without lore slugs
  }
  ...
  for (const entry of entries) {
    ...
    if (result.success) {
      ...
      // Check related links exist (can reference canon or lore slugs)
      for (const relatedSlug of frontmatter.related) {
        if (!validSlugs.has(relatedSlug)) {
          errors.push({
            type: 'error',
            message: `[${relativePath}] Related slug "${relatedSlug}" not found in any canon or lore entry`,
            filepath: entry.filepath,
          });
        }
      }
